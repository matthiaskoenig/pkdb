...
class UploadStudy(APITestCase):
    ...
    def test_upload_reference_json(self):
        ...
        success = upload_reference_json(json_reference, api_url=self.api_url, authentication_header=self.header, client=self.client)
        assert success

    def test_upload_files(self):
        study_name = "test_study"
        study_dir = os.path.join(os.path.dirname(os.path.realpath(__file__)), study_name)
        upload_files(file_path=study_dir, api_url=self.api_url, authentication_header=self.header, client=self.client)
    ...
    def test_upload_study_from_dir(self):
        ...
        setup_database(api_url=self.api_url, authentication_header=self.header, client=self.client)
        upload_study_from_dir(study_dir=study_test_dir, authentication_header=self.header, api_url=self.api_url, client=self.client)
