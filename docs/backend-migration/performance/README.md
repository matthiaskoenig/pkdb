...
One fresh-application run, one excluded warmup, then five sequential measured
complete-study replacements per runtime, on the same machine,
...
Uvicorn. Cold observations and warmup are recorded separately from all five measured warm
samples. Cold upload-to-visible times were legacy: 20.622 s, py313: 2.809 s, py314: 2.385 s.
Cold means a freshly started application with no preparation HTTP request; the
study was already published. OS/database/search caches were not forcibly cleared,
so there is no controlled cold-cache or production-load claim.
...
Use `--cold-start` only immediately after restarting the application. Without it,
the tool still excludes one warmup but does not label any run cold.
