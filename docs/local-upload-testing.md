...
Use the current `pkdb validate` and `pkdb upload` commands against your local backend. All commands below run from the repository root. Only Docker and Docker Compose are required, together with a local study folder.
...
Enter an administrator password at the hidden prompt. Create this account once; subsequent restarts preserve it. Open <http://localhost:18083/docs>.
...
In the API interface, expand `POST /api-token-auth/`, choose **Try it out**, and submit your username and password. Copy the returned `token` value.
...
- Port already in use: set `PKDB_HTTP_PORT` in `.env`, restart Compose, and use that port in browser and curl URLs. Container commands continue using port 8000.
