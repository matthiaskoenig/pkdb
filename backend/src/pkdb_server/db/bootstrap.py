...
def load_vocabulary(session: Session) -> Vocabulary:
    ...
    return project_vocabulary(rows, version.version)


def vocabulary_version(effective, edges, terms) -> str:
    """Identity of the effective authored graph, also used for bundled snapshots."""
...
def vocabulary_from_snapshot(snapshot: Snapshot) -> Vocabulary:
    """Project the exact validation vocabulary a fresh bootstrap would produce."""
    validate_snapshot(snapshot)
    rows = sorted(snapshot.nodes, key=lambda row: row.sid)
    effective = [(row.sid, row.name, row.kind, row.definition) for row in rows]
    edges = sorted((row.sid, parent) for row in rows for parent in row.parents)
    terms = sorted(
        (row.sid, kind, value)
        for row in rows
        for kind, values in row.terms.items()
        for value in set(values)
    )
    return project_vocabulary(rows, vocabulary_version(effective, edges, terms))


def project_vocabulary(rows, version: str) -> Vocabulary:
    return Vocabulary(
        version=version,
        measurements=tuple(
            MeasurementRule.model_validate(
