...
from pkdb_server.api.accounts import require_account
from pkdb_server.api.admin_users import require_administrator
...
async def account_validation_error(request, error):
    ...
    if module == "pkdb_server.api.legacy_uploads" and any(
    ...
    if any(item["type"] == "json_invalid" for item in error.errors()):
        check = None
        if module == "pkdb_server.api.admin_users":
            check = require_administrator
        elif module == "pkdb_server.api.accounts" and path.startswith(
            "/accounts/emails/"
        ):
