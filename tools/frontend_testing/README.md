...
Per-browser reader accounts avoid interfering with the backend's real login throttles. Administrator MFA has deterministic test-only encryption and single-use recovery codes for each browser/retry. No real provider or mail credentials are configured. Traces are off; account screenshots are off to avoid capturing show-once secrets. Scientific failure screenshots and reports remain available.
